...
# -*- coding: utf-8 -*-
"""
CS5100 DA - Assignment 1 

Note: I have removed the header row and names columns from the dataset
      as it does not have a valid floating point value
"""
...
class LrGd:
    def __init__(self,l_rate,fixed_n,stopping_rule):
        self.dataset=[]
        self.splits= 2
        self.l_rate = l_rate
        self.n_epoch = fixed_n
        self.input_file='Auto.csv'
        self.train_set=[]
        self.test_set = []
    ...
    def load_data_from_csv(self):
        ...
        return self.dataset
    '''converting dataset values into float'''
    def data_to_float(self,column):
        for row in self.dataset:
            row[column] = float(row[column].strip())

    '''Adding dummy variables for Origin, Since it is a qualitative variable'''
    def origin_to_quantitatve(self):
        ...
        for row in self.dataset:
            if row[-1]==1.0:
                ...
                row.append(1)
                row.append(1)
    '''TASK - 2: Create a column with 1,0 for mpg and removing mpg from dataset'''
    ...
    def train_model(self):
        ...
        for epoch in range(self.n_epoch):
            ...
            if prev_error_change >= self.stopping_rule:
                break
    '''Use forward propgation to get the value of Pi for MSE computation'''
    ...
    def main(self):
        ...
        for i in range(len(self.dataset[0])):
            self.data_to_float(i)
            
        # Creating high variable, if mpg >=23 1 otherwise 0
        ...
        #Compute MSE
        MSE_train, MSE_test= self.Logit_GD()
        print("\n",MSE_train,"\t",MSE_test)
        return MSE_train, MSE_test

seed_value = 1014  # As said in task 2 - MMDD format from my bday
random.seed(seed_value)
learing_rate=[0.01,0.03,0.07,0.09,0.250,0.500]
fixed_n=[100,250,500,1000]
stopping_rule=100
all_values=[]
for i in range(100):
    for each_epoch_val in fixed_n:
